...
def train_fn(env_name="fruitbot",
    ...
    venv = get_venv(num_envs=num_envs, env_name=env_name, distribution_mode=distribution_mode, num_levels=1, start_level=0)
    # print(venv, env_name, distribution_mode, venv.start_level, venv.num_levels)
    # print(x)

    model = torch.load('results/ppg_16_all_100' + "/model.jd")
    ...
    out = test_roller.multi_step(500)
    x, y, z = test_roller.observe()
    test_roller.multi_step(5000)
    a, b, c = test_roller.observe()
    pkl.dump({"images1":y, "images2":b}, open("datasingle.pkl", "wb"))
